import type {
  SignedTx,
  SolanaTokenInfo,
  X402SvmKitTransaction,
  X402SvmPaymentRequirements,
  X402SvmTokenDisplay,
} from './types'
 ...
 */
export function assertX402Eip3009(message: {
  ...
  if (!Array.isArray(fields)
    ...
    || !EIP3009_TRANSFER_WITH_AUTHORIZATION.every((expected, index) => {
      ...
      return field && typeof field === 'object'
        && (field as Record<string, unknown>).name === expected.name
        && (field as Record<string, unknown>).type === expected.type
    })) {
...
export function x402JsonSafe(value: unknown): any {
  if (typeof value === 'bigint') return value.toString(10)
  if (Array.isArray(value)) return value.map(x402JsonSafe)
  if (value && typeof value === 'object') {
    const result: Record<string, unknown> = {}
    for (const [key, child] of Object.entries(value)) result[key] = x402JsonSafe(child)
    return result
  }
  return value
}

export function x402SignatureHex(result: any): `0x${string}` {
...
export function x402SvmDisplayFields(
  paymentRequirements: X402SvmPaymentRequirements,
  token: X402SvmTokenDisplay,
): { tokenInfo: SolanaTokenInfo[]; tokenRecipientOwners: string[] } {
  if (!paymentRequirements.asset) throw new Error('x402 SVM asset is required')
  if (!paymentRequirements.payTo) throw new Error('x402 SVM payTo is required')
...
function encodeShortVec(value: number): Uint8Array {
  if (!Number.isSafeInteger(value) || value < 0) throw new Error('Invalid Solana shortvec value')
  const bytes: number[] = []
  do {
    let next = value & 0x7f
...
function decodeShortVec(bytes: Uint8Array, start: number): { value: number; next: number } {
  let value = 0
  let multiplier = 1
  let offset = start
  for (let i = 0; i < 5; i++) {
...
export function base64Decode(value: string): Uint8Array {
  ...
  for (let i = 0; i < normalized.length; i += 4) {
    ...
    if (normalized[i + 2] !== '=') bytes.push((chunk >> 8) & 0xff)
    if (normalized[i + 3] !== '=') bytes.push(chunk & 0xff)
  }
  return Uint8Array.from(bytes)
}

/** Build a wire transaction from the compiled transaction supplied by Solana Kit. */
export function x402KitTransactionToWire(transaction: X402SvmKitTransaction): string {
...
export function x402SvmSignatureBytes(result: SignedTx): Uint8Array {
  if (typeof result.signature !== 'string') throw new Error('KeepKey returned no Solana signature')
  const signature = base64Decode(result.signature)
  if (signature.length !== 64) throw new Error(`KeepKey returned a ${signature.length}-byte Solana signature`)
  return signature
}
