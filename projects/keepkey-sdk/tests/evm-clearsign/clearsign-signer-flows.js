...
// Identity logo (1bpp mono RLE) generated by
// keepkey-vault/scripts/clearsign-icons/encode-icon.mjs — sent with the signer
// so the device renders it on the trust screen + every per-tx confirm.
// Set CLEARSIGN_NO_ICON=1 to load a text-only identity instead.
let IDENTITY_ICON = null
if (!process.env.CLEARSIGN_NO_ICON) {
  try {
    IDENTITY_ICON = require('../../../keepkey-vault/scripts/clearsign-icons/generated/identity-icons.json').pioneer
  } catch { /* catalog not generated — fall back to text-only */ }
}
...
function selfProvider() {
  ...
  if (IDENTITY_ICON) {
    ...
    signer.persist = true // exercise the persist-to-flash path (survives reboot)
  }
  return { signer, flows }
}
