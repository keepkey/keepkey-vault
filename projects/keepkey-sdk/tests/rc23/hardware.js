/**
 * RC23 physical-device acceptance tests through the production path:
 * keepkey-sdk -> Vault REST -> hdwallet -> KeepKey.
 *
 * This file is discovered by `test:all`, so it deliberately exits without
 * touching hardware unless RC23_HARDWARE_TEST=1 and an explicit phase are set.
...
const RC23_FULL_FIRMWARE_SHA256 = '92b292eb460c462df9084131a67a78f24fd4061b6d11822401dddd47c07c967f'

const phase = process.argv[2]

if (process.env.RC23_HARDWARE_TEST !== '1') {
  console.log('  SKIP rc23/hardware.js (set RC23_HARDWARE_TEST=1 and choose an explicit phase)')
  process.exit(0)
}

const validPhases = new Set(['taproot', 'p2wpkh-control', 'locked-entropy', 'entropy-budget'])
if (!validPhases.has(phase)) {
  console.error('Usage: npm run test:rc23:hardware -- <taproot|p2wpkh-control|locked-entropy|entropy-budget>')
  process.exit(2)
}

if (!stdin.isTTY || !stdout.isTTY) {
  console.error('RC23 hardware acceptance requires an interactive terminal for operator attestations.')
  process.exit(2)
}

const rl = createInterface({ input: stdin, output: stdout })
const evidence = {
  candidate: 'v7.15.0-rc23',
...
async function connect() {
  const sdk = await KeepKeySdk.create({
    ...
    serviceName: 'KeepKey RC23 Hardware Acceptance',
    serviceImageUrl: '',
  })
  ...
  const expectedFirmwareHash = process.env.RC23_EXPECT_FIRMWARE_HASH
    || (features.firmware_variant === 'KeepKeyZcash' ? RC23_FULL_FIRMWARE_SHA256 : undefined)
  if (expectedFirmwareHash) {
    check('firmware binary hash matches the RC23 artifact', features.firmware_hash === expectedFirmwareHash)
  }
  evidence.firmware = {
...
async function taproot() {
  ...
  if (process.env.RC23_EXPECT_ADDRESS) {
    check('address matches RC23_EXPECT_ADDRESS', hidden.address === process.env.RC23_EXPECT_ADDRESS)
  }
...
main()
  .catch((error) => {
    ...
    console.error('RC23_EVIDENCE_END\n')
    process.exitCode = 1
  })
  .finally(() => rl.close())
