...
describe('firmware capability gate', () => {
  ...
  test('a rejected optional Taproot xpub keeps the three established account choices visible', async () => {
    const calls: string[][] = []
    const rejectingWallet = {
      btcSupportsScriptType: async () => true,
      getPublicKeys: async (paths: any[]) => {
        calls.push(paths.map(path => path.scriptType))
        if (paths.some(path => path.scriptType === 'p2tr')) {
          throw { code: 9, message: 'Invalid combination of coin and script_type' }
        }
        return paths.map((path, i) => ({ xpub: `xpub-${path.scriptType}-${i}` }))
      },
    }

    const manager = new BtcAccountManager()
    await manager.initialize(rejectingWallet)
