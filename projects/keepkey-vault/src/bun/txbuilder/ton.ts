...
function buildInternalMessage(
  ...
  if (memo && memo.length > 0) {
    // TON cells have a 1023-bit data limit; with op(32 bits) the memo can use ~123 bytes
    const memoBytes = Buffer.from(memo, 'utf8')
    if (memoBytes.length > 120) {
      throw new Error(`TON memo too long: ${memoBytes.length} bytes (max 120)`)
    }
...
function readBE(buf: Buffer, offset: number, size: number): number {
  if (offset + size > buf.length) {
    throw new Error(`readBE: buffer overflow at offset ${offset} + ${size} (buf.length=${buf.length})`)
  }
  let val = 0
...
export function buildTonTransfer(params: {
  ...
  // Detect bounce flag from the address tag byte:
  //   0x11 = bounceable, 0x51 = non-bounceable (+0x80 variants for testnet)
  ...
  try {
    ...
    if (rawMatch) {
      ...
      const raw = Buffer.from(b64, 'base64')
      if (raw.length === 36) {
        const tag = raw[0] & 0x3F // mask off testnet bit
        bounce = tag === 0x11
      } else {
        // Invalid length — fall back to string prefix
        bounce = params.to.startsWith('EQ')
      }
    }
  } catch {
    // Decoding failed — fall back to string prefix heuristic
    bounce = params.to.startsWith('EQ') || params.to.includes(':')
  }
...
export async function getTonSeqno(address: string): Promise<number> {
  ...
  if (typeof val === 'string') {
    // Handle "0x..." hex prefix — parseInt with radix 16 expects no prefix
    const hex = val.startsWith('0x') || val.startsWith('0X') ? val.slice(2) : val
    seqno = parseInt(hex, 16)
  } else {
    seqno = Number(val)
  }
