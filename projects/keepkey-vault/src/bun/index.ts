...
// Windows auto-update removed — see docs/RETRO-WINDOWS-AUTO-UPDATE.md
// Windows users are directed to GitHub releases page to download manually.
// This avoids process lock issues where the old app doesn't fully exit
// and the installer can't replace locked DLLs/executables.
	...
	handlers: {
		requests: {
			...
			downloadUpdate: async () => {
				if (process.platform === 'win32') {
					// Windows: open GitHub releases page — user downloads and installs manually
					// (in-app update removed due to process lock issues, see docs/RETRO-WINDOWS-AUTO-UPDATE.md)
					const version = pendingUpdateVersion || Updater.updateInfo()?.version
					const url = version
						? `https://github.com/${GITHUB_REPO}/releases/tag/v${version}`
						: `https://github.com/${GITHUB_REPO}/releases/latest`
					console.log(`[Windows Update] Opening GitHub releases: ${url}`)
			...
			applyUpdate: async () => {
				if (process.platform === 'win32') {
					// Windows: no in-app install — redirect to GitHub
					const version = pendingUpdateVersion || Updater.updateInfo()?.version
					const url = version
						? `https://github.com/${GITHUB_REPO}/releases/tag/v${version}`
						: `https://github.com/${GITHUB_REPO}/releases/latest`
