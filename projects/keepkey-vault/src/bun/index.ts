...
// which only resolves to non-pre-release releases. Pre-releases are invisible.
// This downloads the tar.zst directly from the specific release tag.
...
async function macosDownloadAndInstall(rpc: any) {
	...
	try {
		...
		const appName = path.basename(appBundlePath) // e.g. "keepkey-vault.app"
		const appParent = path.dirname(appBundlePath) // e.g. "/Applications" or wherever
		...
		const mkdirResult = Bun.spawnSync(['mkdir', '-p', stageDir])
		if (mkdirResult.exitCode !== 0) throw new Error('Failed to create staging dir')
		...
		if (!extractedApp || !extractedApp.endsWith('.app')) {
			throw new Error(`No .app found in extracted archive: ${lsResult.stdout.toString()}`)
		}

		console.log(`[macOS Update] Extracted: ${extractedApp}`)

		// Move old app to trash, move new app into place
		...
		requests: {
			...
			downloadUpdate: async () => {
				...
				if (process.platform === 'darwin') {
					// macOS: bypass Electrobun's baseUrl which points to "latest" release.
					// Pre-releases are invisible to it. Download directly from the tag.
			...
			applyUpdate: async () => {
				if (process.platform === 'darwin') {
					// macOS update already applied during download (replace + relaunch).
					// If we get here, the download didn't trigger relaunch — retry.
					await macosDownloadAndInstall(rpc)
					return
				}
				if (process.platform === 'win32') {
