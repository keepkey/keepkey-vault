		...
		<Box position="fixed" inset="0" zIndex={Z.dialog} display="flex" alignItems="center" justifyContent="center" onClick={onClose}>
			...
			<Box position="relative" bg="kk.cardBg" border="1px solid" borderColor="kk.border" borderRadius="xl" p="5" w="520px" maxW="92vw" onClick={(e) => e.stopPropagation()}>
							...
							<Box mt="2" p="3" bg="rgba(255,255,255,0.04)" border="1px solid" borderColor="kk.border" borderRadius="md">
								...
								<Button
									...
									onClick={() => {
										...
										if (url) rpcRequest('openUrl', { url }).catch((e: any) => console.warn('[openUrl]', e?.message))
									}}
								>
				...
				<Flex justify="flex-end" gap="2" mt="4">
					<Button size="sm" variant="ghost" color="kk.textSecondary" px="4" py="2" onClick={onClose}>
						{phase === "broadcast" ? t('done', { ns: 'common' }) : t('cancel', { ns: 'common' })}
					</Button>
					{phase === "input" && (
						<Button size="sm" bg="kk.gold" color="black" px="4" py="2" _hover={{ bg: "kk.goldHover" }} disabled={!canBuild || loading || watchOnly} onClick={handleBuild}>
					...
					{phase === "signing" && (
						<Button size="sm" bg="kk.gold" color="black" px="4" py="2" _hover={{ bg: "kk.goldHover" }} disabled>
							{t('signing')}
						</Button>
					)}
					{phase === "signed" && (
			...
			{loading ? (
				...
				<>
					<Flex gap="2" mb="4">
						...
						>
							<Box as={FaPlus} fontSize="10px" mr="1" />
							{t('delegate')}
						</Button>
						<Button
							size="sm"
						...
						>
							<Box as={FaMinus} fontSize="10px" mr="1" />
							{t('undelegate')}
						</Button>
						{watchOnly && (
