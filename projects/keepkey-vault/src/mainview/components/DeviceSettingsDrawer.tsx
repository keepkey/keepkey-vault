...
import type { DeviceStateInfo, AppSettings, EmulatorStatus } from "../../shared/types"
			...
			>
				...
				<VStack gap="3" align="stretch" p="4">
					...
					{(() => {
						...
						// Not macOS — hide entirely
						if (emuStatus && emuStatus.platform !== 'darwin') return null
						// Status not loaded yet — hide to avoid flash
						if (!emuStatus) return null
						...
						const handlePair = async () => {
							...
							try {
								...
								setEmuStatus(s)
							} catch (e: any) { console.error('emulatorPair:', e) }
							setEmuLoading(false)
						}
						...
						const handleInit = async () => {
							setEmuLoading(true)
							try {
								const s = await rpcRequest<EmulatorStatus>('emulatorInit', undefined, 10000)
								setEmuStatus(s)
							} catch (e: any) { console.error('emulatorInit:', e) }
							setEmuLoading(false)
						}
						...
						const handleStop = async () => {
							...
							try {
								...
								setEmuStatus(s)
							} catch (e: any) { console.error('emulatorStop:', e) }
							setEmuLoading(false)
						}

						return (
							<Section title="Emulator" defaultOpen={false}>
								<VStack gap="3" align="stretch">
									{/* Warning banner */}
									<Box bg="rgba(251,146,60,0.08)" border="1px solid" borderColor="rgba(251,146,60,0.2)" borderRadius="md" px="3" py="2">
										<Text fontSize="xs" color="#FB923C" fontWeight="600">Testing Only</Text>
										<Text fontSize="xs" color="kk.textSecondary" mt="0.5">
											Emulator seeds are for development — never use real funds.
											Flash images are encrypted at rest with macOS Keychain.
										</Text>
									</Box>

									{/* Status row */}
									<Flex justify="space-between" align="center">
										<Text fontSize="sm" color="kk.textSecondary">Status</Text>
										<Flex align="center" gap="2">
											<Box w="8px" h="8px" borderRadius="full" bg={
												emuStatus.state === 'running' ? '#22C55E' :
												emuStatus.state === 'error' ? '#EF4444' :
												emuStatus.paired ? '#C0A860' : 'kk.textMuted'
											} />
											<Text fontSize="sm" color="kk.textPrimary" fontFamily="mono">
												{emuStatus.state === 'running' ? 'Running' :
												 emuStatus.state === 'starting' ? 'Starting...' :
												 emuStatus.state === 'error' ? 'Error' :
												 emuStatus.paired ? 'Paired (idle)' : 'Not paired'}
											</Text>
										</Flex>
									</Flex>
									...
									{/* Error display */}
									{emuStatus.error && (
										<Text fontSize="xs" color="kk.error">{emuStatus.error}</Text>
									)}

									{/* Flash images */}
									{emuStatus.paired && emuStatus.flashImages.length > 0 && (
										<Flex justify="space-between" align="center">
											<Text fontSize="sm" color="kk.textSecondary">Flash images</Text>
											<Text fontSize="sm" color="kk.textPrimary" fontFamily="mono">
												{emuStatus.flashImages.join(', ')}
											</Text>
										</Flex>
									)}
									...
									{emuStatus.paired && (
										<Flex justify="space-between" align="center">
											<Text fontSize="sm" color="kk.textSecondary">Storage</Text>
											<Text fontSize="xs" color="kk.textMuted" fontFamily="mono" maxW="200px" isTruncated>
												{emuStatus.storagePath}
											</Text>
										</Flex>
									)}
									...
									<Flex gap="2" mt="1">
										{!emuStatus.paired ? (
											<Button
												...
												flex="1"
											>
												{emuLoading ? 'Pairing...' : 'Pair with Keychain'}
											</Button>
										) : emuStatus.state === 'running' ? (
											<Button
												size="sm"
												...
												flex="1"
											>
												{emuLoading ? 'Stopping...' : 'Stop Emulator'}
											</Button>
										) : (
											<Button
												size="sm"
												...
												flex="1"
											>
												{emuLoading ? 'Starting...' : 'Start Emulator'}
											</Button>
										)}
									</Flex>
								</VStack>
