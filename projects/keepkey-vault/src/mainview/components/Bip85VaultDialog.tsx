...
export function Bip85VaultDialog({ onClose }: Bip85VaultDialogProps) {
	...
	const clearMnemonic = useCallback(() => {
		...
		if (autoClearTimer.current) { clearTimeout(autoClearTimer.current); autoClearTimer.current = null }
	}, [])
	...
	/** Derive a new seed — optimistic UI + background DB save */
	const handleCreate = useCallback(async () => {
		const idx = nextIndex(wordCount)
		const seedLabel = label.trim() || `Seed ${savedSeeds.length + 1}`
		clearMnemonic()
		setDeriving(true)
		try {
			const result = await rpcRequest<{ mnemonic: string; derivationPath: string }>(
				"getBip85Mnemonic",
			...
			setCreating(false)
			setLabel("")
			autoClearTimer.current = setTimeout(() => clearMnemonic(), 60000)
		} catch (e: any) {
			setError(e?.message || t("bip85.derivationFailed"))
		} finally {
			setDeriving(false)
		}
	}, [wordCount, label, savedSeeds, nextIndex, clearMnemonic, t])

	/** Re-derive an existing saved seed (click on history item) */
	...
	const handleDelete = useCallback(async (seed: Bip85SeedMeta) => {
		try {
			...
			if (activeViewKey === `${seed.wordCount}-${seed.index}`) clearMnemonic()
		} catch (e: any) {
			console.warn("Failed to delete seed:", e.message)
		}
		setConfirmDeleteKey(null)
	}, [activeViewKey, clearMnemonic])
		...
		<>
			...
			>
				...
				<Box flex="1" overflow="auto" px="5" py="4">
					...
					{/* Mnemonic display — shown when viewing any seed */}
					{mnemonic && (
						<Box bg="rgba(255,255,255,0.03)" border="1px solid" borderColor="kk.gold" borderRadius="lg" p="4" mb="4">
							<Text fontSize="xs" color="kk.textMuted" fontFamily="mono" mb="2">{derivationPath}</Text>
							<Box
								position="relative" p="3" bg="rgba(0,0,0,0.3)" borderRadius="md"
								style={{ filter: revealed ? "none" : "blur(8px)", userSelect: revealed ? "text" : "none", WebkitUserSelect: revealed ? "text" : "none" }}
							>
								<Flex flexWrap="wrap" gap="1.5">
									{mnemonic.split(" ").map((word, i) => (
										<Text key={i} fontSize="xs" fontFamily="mono" color="kk.textPrimary">
											<Text as="span" color="kk.textMuted" fontSize="10px">{i + 1}.</Text>{word}
										</Text>
									))}
								</Flex>
							</Box>
							<Text fontSize="xs" color="kk.textMuted" mt="2" textAlign="center" fontStyle="italic">{t("bip85.autoClearNotice")}</Text>
							<Flex gap="2" mt="3" justify="center">
								<Button size="xs" variant="outline" borderColor="kk.border" color="kk.textSecondary" _hover={{ bg: "rgba(255,255,255,0.06)" }} onClick={() => setRevealed(!revealed)}>
									{revealed ? t("bip85.hide") : t("bip85.reveal")}
								</Button>
								<Button size="xs" variant="outline" borderColor="kk.border" color={copied ? "kk.success" : "kk.textSecondary"} _hover={{ bg: "rgba(255,255,255,0.06)" }} onClick={handleCopy} disabled={!revealed}>
									{copied ? t("bip85.copied") : t("bip85.copy")}
								</Button>
								<Button size="xs" variant="outline" borderColor="kk.border" color="kk.textSecondary" _hover={{ bg: "rgba(255,255,255,0.06)" }} onClick={clearMnemonic}>
									{t("bip85.dismiss")}
								</Button>
							</Flex>
						</Box>
					...
					{!creating ? (
						<Button
							...
							disabled={deriving}
							onClick={() => { setCreating(true); clearMnemonic() }}
						>
						...
						<Box bg="rgba(255,255,255,0.03)" border="1px solid" borderColor="kk.border" borderRadius="lg" p="4" mb="4">
							...
							>
								{deriving ? t("bip85.deriving") : `Derive ${wordCount}-word seed`}
							</Button>
						</Box>
					)}
						...
						{loadingSeeds ? (
							...
							<Flex direction="column" gap="1.5">
								{savedSeeds.map((seed) => {
									...
									const isActive = activeViewKey === key && mnemonic
									...
									return (
										...
										>
											...
											<Flex align="center" gap="1.5" flexShrink={0}>
												{isActive && (
													<Text fontSize="10px" color="kk.gold" fontWeight="600">VIEWING</Text>
												)}
